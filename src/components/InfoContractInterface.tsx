...
const InfoContractInterface = () => {
  ...
  // 获取ETH余额
  const getBalanceETH = useCallback(async () => {
    if (!contract) return;
    setLoadingEth(true);
    setErrorEth("");
    try {
  ...
  const [isApproveSuccess] = useState(false);

  // 获取ETH余额
  const getBalanceYD = useCallback(async () => {
    if (!contract) return;
    setLoadingYd(true);
    setErrorYd("");
    try {
  ...
  const handleApproveYD = async () => {
    ...
    try {
      ...
      const ydContract = YidengCoin__factory.connect(ERC20_ADDRESS, signer as unknown as Signer);
    ...
    <div className="max-w-md mx-auto p-4 border border-gray-300 rounded-md">
      ...
      <div className="mb-6">
        ...
        <div className="mb-4 flex items-center">
          ...
          {loadingEth ? <p>加载中...</p> : errorEth ? <p className="text-red-500">错误: {errorEth}</p> : <p>{`${ethBalance || 0} ETH`}</p>}
        </div>
      ...
      <div className="mb-6">
        ...
        <div className="mb-4 flex items-center">
          ...
          {loadingYd ? <p>加载中...</p> : errorYd ? <p className="text-red-500">错误: {errorYd}</p> : <p>{`${ydBalance || 0} YD`}</p>}
        </div>
        <div className="space-y-2">
